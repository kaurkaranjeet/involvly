    ...
    public function GetStudentsByClass(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $class = User::with('SchoolDetail:id,school_name')
                            ...
                            ->select('users.*', 'class_code.class_name')->where('role_id', 2)->where('user_class_code.class_id', $request->class_id)->where('user_class_code.active', '1')->get();
            return response()->json(array('error' => false, 'message' => 'Record found', 'data' => $class), 200);
        }
    }
...
//            $assignment = Assignment::with('AssignedClass')->where('teacher_id', $request->teacher_id)->get();
    ...
    public function GetAssignmentDetails(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $getData = SubmittedAssignments::select('submitted_attachement')->with('subjects')->with('Assignments.User')->where('student_id', $request->student_id)->where('class_id', $request->class_id)->where('submit_status', '1')->orderBy('id', 'DESC')->get();
