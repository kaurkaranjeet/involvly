    ...
    public function CustomMembers(Request $request) {
      try {
        ...
        if ($validator->fails()) {
         ...
         $group_data= User::where('role_id',3)->where('status',1)->where('school_id',$usrobj->school_id)->whereRaw('id NOT IN( Select to_user_id FROM report_users WHERE from_user_id='.$user_id.')')->whereRaw('id NOT IN( Select from_user_id FROM report_users WHERE to_user_id='.$user_id.')')->get();   
         $array=array('error' => false, 'data' => $group_data);
      return response()->json($array, 200);
       }
     } catch (\Exception $e) {
