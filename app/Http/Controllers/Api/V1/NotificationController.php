	...
	public function AllNotifications(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			if ($request->type == 'school_notification') {
				...
				->where('type', 'school_notification')
				->select(DB::raw("DATE(created_at) as notification_date"), "notification.*")
				->with('User:id,name,role_id,profile_image')
				->orderBy('id', 'DESC')
				->get();
