...
$parents =  ParentChildrens::where('children_id',$request->student_id)->select('parent_childrens.*, (SELECT group_concat(u.name) from  users as u where  u.id=parent_childrens.parent_id)  as childrens')->with('ParentDetails:id,name,first_name,last_name,role_id')->get();
