...
//         $results= ParentChildrens::select(DB::raw('DISTINCT parent_id'))->with('ParentDetails')->whereRaw('children_id IN('.$childrens.')')->where('parent_id','!=',$request->parent_id)->get();
        ...
        $id=$request->parent_id;
         $results=DB::select( DB::raw("select id,name,role_id FROM users where family_code='".$request->family_code."'  AND role_id=3  ORDER BY FIELD(id, ".$id.") DESC " ));
        ...
        $children= User::with('SchoolDetail:id,school_name')->whereRaw('family_code ="'.$request->family_code.'" AND role_id = 2')->select('id','name','last_name','role_id','school_id',DB::raw('select active as school_status where user_id=users.id limit 1'))->get();
