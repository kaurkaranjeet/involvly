 ...
 WHERE to_user_id=".$request->user_id.")")->orderBy('id', 'DESC')->get();
    ...
    public function GetPostHomefeedDescription(Request $request){
     ...
     $posts = Post::select((DB::raw("( CASE WHEN EXISTS (
              ...
              ELSE FALSE END)
              AS is_like,posts.*")))->with('user','user.CityDetail','user.SchoolDetail','user.StateDetail')->withCount('likes','comments')->where('id', $request->post_id)->orderBy('id', 'DESC')->get();
    } 
    return response()->json(array('error' => false, 'message' => 'Record found', 'data' => $posts), 200);
...
//          total reply comments
      ...
      if($user_name->id!=$request->user_id){
       ...
       $notifications = Notification::create(['user_id'=>$user_name->id,'from_user_id'=>$request->user_id,'notification_message'=>$message,'type'=>'social_notification','notification_type'=>'comment','push_type'=>'add_comment','post_id'=>$request->post_id]);
       $notifications->role_type = 'all';
      $this->pusher->trigger('notification-channel', 'notification_all_read', $notifications);
     }
