...
class UserController extends Controller {
    ...
    public function manageUsers(Request $request, $id) {
        ...
        if ($request->type == 'teacher') {
            $users = User::where('role_id', 4)->where('status', 1)->where('school_id', $id)->select(DB::raw('(select GROUP_CONCAT(u.class_code) AS class_codes from assigned_teachers inner join class_code as u ON assigned_teachers.class_id=u.id where user_id=users.id) as class_codes ,users.*'))->get();
